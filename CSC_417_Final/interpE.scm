...
(define desugar-let
  (lambda (letform env)
    (if (not (and (list? letform) 
		  (eq? 'let (car letform))
		  (= 4 (length letform))))
	(list 'error "Expected properly constructed 'let' form" letform)
...
(define env-entry-maker name->cell)
...
(define do-application
  (lambda (exp env)
    (let ((rator (ev (car exp) env))
      ...
      (cond 
       ((function? rator)
	(apply-function rator rands env))
       ((built-in? rator)
  ...
  (lambda (exp env)
    (let ((keyword (car exp))
      ...
      (cond ((eq? keyword 'if)
	    ...
	    ((eq? keyword '*)
	     (let ((arg (car rest)))
	       (let ((maybe-lvalue (ev arg env)))
		 (if (cell? maybe-lvalue)
		     (cell-ref maybe-lvalue)
		     (list 'error "Cannot deref a value that is not a cell)" exp)))))
	    ((eq? keyword '&)
	     ;; Get the lvalue for the argument to the deref operator, &
	     (let ((arg (car rest)))
	       (let ((value (ev arg env)))
		 (make-cell value))))
	    (#t
...
(define test
  (lambda ()
    (let ((ev1 (lambda (exp) (ev exp (make-env)))))
      ...
      ;; 'alter' works in this interpreter
      (assert (run '(let x 1
		      (let dummy (alter x 100)
			x)))
	      100)

      ;; 'alter' works on global variables also
      (assert (run 'x) 10)
      (assert (run '(let foobar 1
		      (let dummy (alter x 999)
			x)))
	      999)

      ;; can write functions that expect references as args
      (assert (run '(let foo (fn (p1 p2) 
				 (add (* p1) (* p2)))
		      (foo (& x) (& (mul 3 4)))))
	      22)

      ;; if we alter a reference, we must change to another reference
      (assert (run '(let foo (fn (p1 p2) 
				 (let dummy (alter p1 (& 100))
				   (add (* p1) (* p2))))
		      (foo (& x) (& (mul 3 4)))))
	      112)
      ...
      ;; if we alter a reference, we must change to another reference
      (assert (run '(let foo (fn (p1 p2) 
				 (let dummy (alter p2 (& 100))
				   (add (* p1) (* p2))))
		      (foo (& x) (& (mul 3 4)))))
	      110)

      ;; the reference we alter could be to an expression, as in the previous test
      (assert (run '(let var (mul 3 5)
		      (let dummy (alter var 9)
			var)))
	      9)

      
      ;; dynamic typing in action: var starts out holding a reference, but we
      ;; alter it to hold a value, which we return
      (assert (run '(let var (& (mul 3 5))
		      (let dummy (alter var 9)
			var)))
	      9)

      ;; dynamic typing in action: var starts out holding a value, but we
      ;; alter it to hold a reference, whose type we check with 'cell?'
      (assert (cell? (run '(let var (mul 3 5)
			     (let dummy (alter var (& 9))
			       var)))))

      ;; and that cell from the previous test contains 9, as expected
      (assert (cell-ref (run '(let var (mul 3 5)
				(let dummy (alter var (& 9))
				  var))))
	      9)
      )))
