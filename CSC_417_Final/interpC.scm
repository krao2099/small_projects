...
(define name->existing-cell		     ; [[4]]
  (lambda (name value)
    (cons name (if (cell? value)
		   value
		   (make-cell value)))))

(define env-entry-maker name->existing-cell) ; [[5]]

(define make-env
  (lambda ()
    (map env-entry-maker 		     ; [[6]]
...
(define prep-operand get-or-make-cell)	     ; [[7]]
